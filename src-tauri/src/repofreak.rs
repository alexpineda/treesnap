...
async fn do_codefetch(cfg: CodefetchConfig) -> Result<String> {
    ...
    if codefetchignore.exists() {
        ignore_builder
            .add(codefetchignore)
            .context("Failed to parse .codefetchignore")?;
    }
    ...
    let bpe = if cfg.max_tokens.is_some() {
        ...
        let mut bpe_builder = tiktoken_rs::BpeBuilder::new();
        // We'll grab p50k base from the compiled JSON in the tiktoken-rs crate
        // (or you can embed your own if you want).
        let core_bpe = bpe_builder.model("p50k_base").build()?;
        Some(core_bpe)
    } else {
        None
    };
...
pub fn run() {
    tauri::Builder::default()
        .invoke_handler(tauri::generate_handler![run_codefetch])
